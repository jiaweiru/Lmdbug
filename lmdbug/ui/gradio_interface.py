...
class LmdbugInterface:
    ...
    def _load_protobuf(self, protobuf_module: str, message_class: str) -> tuple[list[str], str, str]:
        ...
        try:
            ...
            if config_changed:
                logger.info(f"Updating protobuf config: {message_class}")
    ...
    def _reload_protobuf_internal(self):
        ...
        try:
            ...
            logger.info("Protobuf configuration reloaded after database change")
        except Exception as e:
    ...
    def _preview_first_entries(self, count: int) -> tuple[list[dict], str]:
        """Preview the first N entries."""
        if error := self._check_service_loaded():
            return [], error

        results = self.preview_service.preview_first_entries(count)
        return results, f"Showing first {len(results)} entries"

    def _browse_by_index(self, start_index: int, count: int) -> tuple[list[dict], str]:
    ...
    def _search_exact_key(self, key: str) -> tuple[list[dict], str]:
        ...
        if "error" in result:
            return [], f"Key search failed: {result['error']}"
        return [result], f"Found exact key: {key}"

    def _search_by_prefix(self, prefix: str, limit: int) -> tuple[list[dict], str]:
        ...
        return results, f"Found {len(results)} keys with prefix '{prefix}'"

    def _search_by_pattern(self, pattern: str, limit: int) -> tuple[list[dict], str]:
        ...
        return results, f"Found {len(results)} keys matching pattern '{pattern}'"
