...
class PreviewService:
    ...
    def load_protobuf_modules(
        ...
        for module_config in modules:
            ...
            try:
                ...
                loaded_count += 1
                logger.debug(f"Loaded {message_class} from {module_path}")
        ...
        if errors:
            raise ProtobufError(
                f"Failed to load {len(errors)} modules: {'; '.join(errors)}"
            )
    ...
    def _add_protobuf_info(
        ...
        try:
            ...
            result["protobuf_error"] = (
                f"Failed to deserialize with type '{message_type}': {str(e)}"
            )
        except Exception as e:
            logger.error(
            ...
            result["protobuf_error"] = (
                f"Unexpected error with type '{message_type}': {str(e)}"
            )
    ...
    def set_field_config(self, message_type: str, config: dict[str, dict]):
        ...
        logger.debug(f"Updated field config for {message_type}")
    ...
    def load_custom_processors(self, processor_file_path: str) -> None:
        """Loads custom processors from a Python file using global decorator system.

        Args:
            processor_file_path: Path to the Python file containing @custom_processor decorators.
        ...
        loaded_count = processor_registry.load_from_file(processor_file_path)
    ...
    def load_field_config_from_file(self, config_file_path: str) -> dict:
        ...
        if not isinstance(config, dict):
            raise ValueError(
                f"Config file {config_file_path} must contain a JSON object at root level"
            )
