...
class LmdbugConfig:
    ...
    # Protobuf settings
    protobuf_module_path: Optional[str] = None
    protobuf_message_class: Optional[str] = None
    
    # Processor settings
    processor_paths: List[str] = field(default_factory=lambda: ["config_examples/custom_processors.py"])
    ...
    def validate_protobuf_config(self) -> None:
        ...
        if self.protobuf_module_path and not self.protobuf_message_class:
            raise ValueError("protobuf_message_class is required when protobuf_module_path is provided")
        
        if self.protobuf_module_path and not Path(self.protobuf_module_path).exists():
            logger.warning(f"Protobuf module does not exist: {self.protobuf_module_path}")
    ...
    @property
    def protobuf_config_dict(self) -> Optional[Dict[str, str]]:
